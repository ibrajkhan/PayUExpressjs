...
app.use(bodyParser.urlencoded({ extended: false })); // ✅ Fix: to handle form-urlencoded
app.use(bodyParser.json());
app.use(express.urlencoded({ extended: true }));
...
app.post("/payu-success", async (req, res) => {
  ...
  try {
    await payment.save();
    console.log("✅ Payment saved to DB:", txnid);
  } catch (err) {
  ...
  res.send("Payment processed and email sent.");
});
